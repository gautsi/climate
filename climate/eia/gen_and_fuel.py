...
def get_gen_and_fuel(dest_folder):
    ...
    gf_df["year_month"] = gf_df.apply(lambda row: dt.datetime(year = row["year"], month=row["month"], day=1), axis=1)
    return gf_df
...
def get_long_form(gf_df):
    ...
    prefix_specs = [
        {
            "prefix": p,
            "month_fields": [f"{p}_{i}" for i in month_names]
    ...
    return add_plant_op_names(lng_df, gf_df)